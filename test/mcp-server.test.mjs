...
test("MCP protocol lists tools and scopes timer operations to _meta.threadId", async () => {
  const child = spawn(process.execPath, ["bin/codex-timer-mcp.mjs"], {
...
test("scheduling fails clearly without Codex thread metadata", async () => {
  const child = spawn(process.execPath, ["bin/codex-timer-mcp.mjs"], {
    cwd: new URL("..", import.meta.url),
    stdio: ["pipe", "pipe", "pipe"],
  });
